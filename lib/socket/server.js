...
/** Po zerwaniu socketu użytkownik zostaje w pokoju — reconnect bez „opuścił czat”. */
...
function addRoomUser(room, username, avatarSeed, avatarStyle) {
...
function getActiveRoomsList() {
  return Object.keys(rooms).map((roomName) => ({
    roomName,
    isOpen: isOpenRoom(rooms[roomName]),
...
function cancelGraceLeave(room, username) {
  ...
  const timer = room.graceTimers.get(username);
  if (timer) clearTimeout(timer);
  room.graceTimers.delete(username);
}
...
function finalizeGraceLeave(io, roomName, username) {
  const room = rooms[roomName];
  if (!room || !room.users.has(username)) return;

  cancelGraceLeave(room, username);
...
function scheduleGraceLeave(io, socket, roomName, username) {
  ...
  const existing = room.graceTimers.get(username);
  if (existing) clearTimeout(existing);

  const timer = setTimeout(() => {
    finalizeGraceLeave(io, roomName, username);
  }, DISCONNECT_GRACE_MS);
  if (typeof timer.unref === 'function') timer.unref();
  room.graceTimers.set(username, timer);
}

/** Ten sam nick po grace / krótkim offline — bez zajęcia #1234. */
function resolveJoinUsername(room, requestedUsername) {
  const base = normalizeRequestedUsername(requestedUsername);
  if (!base) return null;
  if (room.users.has(base)) return base;
...
function handleJoinRoom(socket, io, {
  ...
  if (rooms[roomName]) {
    ...
    const room = rooms[roomName];
    const assignedUsername = resolveJoinUsername(room, username);
    if (!assignedUsername) {
      socket.emit('roomError', { roomName, message: 'Nieprawidłowa nazwa użytkownika.' });
      return false;
    }

    const reclaiming = room.users.has(assignedUsername);
    cancelGraceLeave(room, assignedUsername);
    ...
    if (reclaiming) {
      ...
      addRoomUser(room, assignedUsername, avatarSeed, avatarStyle);
    }
    ...
    addRoomUser(rooms[roomName], assignedUsername, avatarSeed, avatarStyle);
...
function initSocketIO(httpServer) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', ({ roomName, password, username, restore, noPassword, avatarSeed, avatarStyle }) => {
    ...
    socket.on('restoreRooms', ({ rooms: rawRooms, username, avatarSeed, avatarStyle }) => {
