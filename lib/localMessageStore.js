...
function imageMatchesChannel(item, channelId) {
  const active = normalizeRoomChannelId(channelId);
  const stored = normalizeRoomChannelId(item?.roomChannelId);
  if (!active) return !stored;
  if (!stored) return false;
  return stored === active;
}

export function saveLocalImage(roomName, imageId, { data, mime, sender, timestamp, roomChannelId = '' }) {
  const room = normalizeRoomName(roomName);
  if (!room || !isValidImageId(imageId)) return;
  if (!validateImageEntry({ data, mime })) return;

  const store = loadStore();
  store[storeKey(room, imageId)] = {
    data,
    mime,
    sender,
    timestamp,
...
export function getLocalImage(roomName, imageId, roomChannelId = '') {
  const room = normalizeRoomName(roomName);
  const item = loadStore()[storeKey(room, imageId)];
  if (!item || !validateImageEntry(item)) return null;
  if (!imageMatchesChannel(item, roomChannelId)) return null;
  return `data:${STORE_MIME};base64,${item.data}`;
}

export function getLocalImagesForRoom(roomName, roomChannelId = '') {
  const store = loadStore();
  const prefix = `${normalizeRoomName(roomName)}:`;
  return Object.entries(store)
    .filter(([key, item]) => key.startsWith(prefix) && imageMatchesChannel(item, roomChannelId))
    .map(([key, item]) => ({
...
export function clearLocalImagesForRoom(roomName, { exceptChannelId = null } = {}) {
  const room = normalizeRoomName(roomName);
  if (!room) return;
  const exceptChannel = exceptChannelId != null ? normalizeRoomChannelId(exceptChannelId) : null;
  const store = loadStore();
  const prefix = `${room}:`;
  let changed = false;
  for (const key of Object.keys(store)) {
    const keyRoom = key.includes(':') ? key.slice(0, key.indexOf(':')) : '';
    if (!key.startsWith(prefix) && keyRoom !== room) continue;
    if (exceptChannel && imageMatchesChannel(store[key], exceptChannel)) continue;
    delete store[key];
    changed = true;
  }
