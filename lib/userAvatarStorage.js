...
export function readStoredUserAvatarSeed() {
  ...
  return secureLocalGetSync(USER_AVATAR_SEED_KEY)?.trim() || '';
}

export function readStoredUserAvatarStyle() {
  if (typeof window === 'undefined') return DEFAULT_USER_AVATAR_STYLE;
  return normalizeUserAvatarStyle(secureLocalGetSync(USER_AVATAR_STYLE_KEY));
}

export function writeStoredUserAvatar(seed, style = DEFAULT_USER_AVATAR_STYLE) {
  if (typeof window === 'undefined') return;
  secureLocalSetSync(USER_AVATAR_SEED_KEY, seed);
  secureLocalSetSync(USER_AVATAR_STYLE_KEY, normalizeUserAvatarStyle(style));
}

export function clearStoredUserAvatar() {
  if (typeof window === 'undefined') return;
  secureLocalRemove(USER_AVATAR_SEED_KEY);
  secureLocalRemove(USER_AVATAR_STYLE_KEY);
}
...
/** Odczyt avatara do UI / join — bez zapisu losowego seeda (unika race przy „Wejdź”). */
export function readProfileAvatarFromStorage() {
...
export function getJoinAvatarPayload(fallbackSeed = '') {
