...
const VV_HEIGHT_VAR = '--mobile-vv-height';
const VV_OFFSET_TOP_VAR = '--mobile-vv-offset-top';

let mobileChatViewportCleanup = null;
...
export function syncMobileChatVisualViewport() {
  ...
  root.style.setProperty(VV_HEIGHT_VAR, `${Math.round(vv.height)}px`);
  root.style.setProperty(VV_OFFSET_TOP_VAR, `${Math.round(vv.offsetTop || 0)}px`);
}

function attachMobileChatViewportSync() {
  detachMobileChatViewportSync();
  ...
  mobileChatViewportCleanup = () => {
    vv?.removeEventListener('resize', update);
    vv?.removeEventListener('scroll', update);
    window.removeEventListener('resize', update);
    window.removeEventListener('orientationchange', update);
    document.documentElement.style.removeProperty(VV_HEIGHT_VAR);
    document.documentElement.style.removeProperty(VV_OFFSET_TOP_VAR);
  };
}

function detachMobileChatViewportSync() {
  mobileChatViewportCleanup?.();
  mobileChatViewportCleanup = null;
}
